...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	public DisplayIMGUI[] videosToPlay;
	public bool shouldShowVideos;

	public Text[] productLabels;
	public bool shouldShowProductLabels;

	public Image[] productImageLabels;
	public bool shouldShowProductImageLabels;
	...
	void TurnOffNonactiveVideos()
	{
		...
		for (int i = 0; i < _pageCount; i++) {
			...
			if (shouldShowVideos) videosToPlay [i].gameObject.SetActive (false);
			if (shouldShowProductLabels)  productLabels [i].gameObject.SetActive (false);
			//if (shouldShowProductImageLabels)  productImageLabels [i].gameObject.SetActive (false);
		}
	}
    ...
    void Update() {
        ...
        if (_lerp) {
            ...
            if (_showPageSelection) {
                SetPageSelection(GetNearestPage());  // this is where you go to a selected page
            }
        }
    }
	...
	{  // DH:  I HOPE THIS IS WHERE you go to next page
	...
	// -=============================================
	public void StopPreviousPageVideo (int aPageIndex)
	{
//		Debug.Log ("Video Array Size " + videosToPlay.Length);
//		Debug.Log ("Page video = " + aPageIndex + videosToPlay[aPageIndex] );
		//videosToPlay [aPageIndex]._mediaPlayer.Stop ();
		//videosToPlay[aPageIndex]._mediaPlayer.Rewind (true);  // pause

		if (shouldShowVideos) videosToPlay [aPageIndex].gameObject.SetActive (false);
    	if (shouldShowProductLabels) productLabels [aPageIndex].gameObject.SetActive (false);
//		if (shouldShowProductImageLabels) productImageLabels [aPageIndex].gameObject.SetActive (false);
	}


	public void StartNewPageVideo (int aPageIndex)
	{
		aPageIndex = Mathf.Clamp(aPageIndex, 0, _pageCount - 1);

		if (shouldShowVideos ) videosToPlay [aPageIndex].gameObject.SetActive (true);
		//videosToPlay[aPageIndex]._mediaPlayer.Play ();  //DH
		if ( shouldShowProductLabels) productLabels [aPageIndex].gameObject.SetActive (true);
		if ( shouldShowProductImageLabels) productImageLabels [aPageIndex].gameObject.SetActive (true);
	}

    //------------------------------------------------------------------------
    private void NextScreen()
	{
		StopPreviousPageVideo (_currentPage);

        LerpToPage(_currentPage + 1);
		StartNewPageVideo (_currentPage + 1);
		//_currentPage++;
    }
	...
	{
		print ("Page - " + requestedPage);

		StopPreviousPageVideo (_currentPage);
		LerpToPage(requestedPage);
		StartNewPageVideo (requestedPage);
